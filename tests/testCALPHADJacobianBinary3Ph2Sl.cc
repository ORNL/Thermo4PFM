...
TEST_CASE("CALPHAD Jacobian binary 3Ph2Sl", "[CALPHAD Jacobian binary 3Ph2Sl]")
{
    ...
    std::cout << "-----------------------------------------" << std::endl;
    std::cout << "Test Jacobian for CALPHADConcSolverBinary3Ph2Sl...\n";
    {
        ...
        // test 3 cases for phi values
        double test0[3] = { 0.2, 0.3, 0.5 }; // hphi2 > 0.4
        double test1[3] = { 0.2, 0.5, 0.3 }; // hphi1 > 0.4
        double test2[3] = { 0.5, 0.3, 0.2 };

        double* phi_ptr[3] = { &test0[0], &test1[0], &test2[0] };

        for (int it = 0; it < 3; it++)
        {
            double hphi0 = phi_ptr[it][0];
            double hphi1 = phi_ptr[it][1];
